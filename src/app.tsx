...
export class App extends React.Component<unknown, AppState> {
    ...
    override render(): JSX.Element {
        return (
            <ThemeProvider theme={this.applicationTheme}>
                <Grid className="container" container={true} alignItems="center" justifyContent="center">
                    <Grid item={true}>
                        <Card className="card-container" elevation={0}>
                            ...
                            >
                                <InputComponent
                                    onResultCallback={(result: PassCrackResult) =>
                                        this.setState({ ...this.state, result: result })
                                    }
                                    clearResults={() => this.setState({ ...this.state, result: undefined })}
                                />
                                <PasswordStrengthDisplayComponent
                                    score={this.state.result?.score ?? 0}
                                    guesses={this.state.result?.guesses ?? 0}
                                    attackTime={this.state.result?.attackTime}
                                    warning={this.state.result?.feedback.warning}
                                    suggestions={this.state.result?.feedback.suggestions}
                                />
                            </Grid>
                        </Card>
                    </Grid>
